...
- When the delete button is clicked:
  ...
  3. Show a confirmation dialog: "Ban co chac chan muon xoa du an '{name}' khong? Hanh dong nay
     khong the hoan tac."
  4. If the user confirms (Yes), use `shutil.rmtree` to permanently delete the project folder from
     `self.workspaces_dir`.
