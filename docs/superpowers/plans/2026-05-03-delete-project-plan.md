...
**Architecture:** We will modify the Tkinter UI in `src/ui/dashboard.py` to add a delete button next
to the "Recent Projects" heading. The button will trigger a confirmation dialog, and if confirmed,
use `shutil.rmtree` to delete the project folder from the disk.
...
In `src/ui/dashboard.py`, inside the `_build_ui` method, replace the single "Du an gan day" label
(around line 61) with a frame containing the label and the new button.
...
Add the `delete_selected_project` method to the `DashboardApp` class in `src/ui/dashboard.py` (e.g.,
right after `show_create_dialog`).
...
Since this is a UI-heavy change involving file system operations, a manual smoke test is the most
reliable verification. Run: `python make.py run` (or `python src/ui/dashboard.py`). Expected:
