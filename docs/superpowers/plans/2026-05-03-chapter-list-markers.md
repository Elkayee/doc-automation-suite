...
**Goal:** Add per-chapter and per-subchapter configurable bullet markers by nesting level, and apply
them consistently in the visual builder preview and DOCX export.

**Architecture:** Store per-file settings in `config.yaml` under
`settings.chapter_settings.<filename>`. Add a focused helper for chapter-scoped settings lookup and
list-marker resolution. Keep markdown syntax standard in the editor, then render/export with
configured markers based on indentation level.
...
- [ ] **Step 2: Run test to verify it fails**
- [ ] **Step 3: Extend preview parsing/rendering to keep list items separate and apply configured
      markers**
- [ ] **Step 4: Run test to verify it passes**
