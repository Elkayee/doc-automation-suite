...
## 2026-06-04 - Memory Efficient Line Counting

**Learning:** Using `str.splitlines()` or `len(text.splitlines())` on large strings causes
significant O(N) memory allocations by creating an intermediate list of all lines.

**Action:** Use `text.count('\n') + (1 if text and not text.endswith('\n') else 0)` to count lines
in large strings, which is significantly faster and avoids allocating an intermediate list.
