...
## 2024-05-30 - Optimize str.splitlines() with count()

**Learning:** `str.splitlines()` eagerly allocates a full list of all lines in memory, which is
inefficient when we only need the line count. **Action:** Replace `len(text.splitlines())` with
`text.count('\n') + (1 if text and not text.endswith('\n') else 0)` when only the count is needed,
to avoid memory allocation and speed up execution.
