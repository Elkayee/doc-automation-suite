...
**Learning:** Inserting dummy text into a Tkinter `Listbox` to act as an empty state pollutes the data model, leads to bad click behavior, and creates false affordances.
**Action:** Use a dedicated `ttk.Label` for the empty state and toggle its visibility against the `Listbox` using `pack()`/`pack_forget()`. Wrap them in a container frame so toggling doesn't break the parent layout.
