...
**Learning:** In Tkinter, avoiding empty list states by inserting dummy placeholder strings into a `tk.Listbox` pollutes the data model and creates false affordances.
**Action:** Wrap the Listbox and a dedicated `ttk.Label` in a `ttk.Frame`, and use `pack()`/`pack_forget()` to toggle their visibility based on the data state.
