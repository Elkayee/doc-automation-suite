...
## 2023-10-25 - Tkinter Keyboard Accessibility
...
<Return> key). **Action:** To ensure keyboard accessibility, explicitly call `.focus_set()` on
primary entry fields when opening dialogs, and bind `<Return>` events to primary actions (with an
optional event parameter on the handler).
