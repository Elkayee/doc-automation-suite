...
## 2024-05-24 - Handle keyboard accessibility in Tkinter dialogs and lists

**Learning:** Toplevel dialogs and list components do not automatically provide keyboard navigation
support (such as auto-focusing inputs or submitting actions via `<Return>`). **Action:** Explicitly
call `.focus_set()` on primary entry fields when opening dialogs, and bind `<Return>` events to
primary actions.
