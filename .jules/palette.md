...
## 2024-06-12 - Enhance Tkinter keyboard accessibility with focus and Enter bindings
...
`<Return>` key). **Action:** Explicitly call `.focus_set()` on primary entry fields when opening
dialogs, and bind `<Return>` events to primary actions (ensuring the action function accepts an
