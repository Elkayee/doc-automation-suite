...
**Learning:** Destructive actions (like "Xoa Du An") should visually indicate when they are not applicable by using a disabled state. Previously, the button was always clickable, but would show a warning dialog if clicked without a selection.
**Action:** Add disabled state to the button initially and bind listbox select event to toggle disabled state based on selection.
