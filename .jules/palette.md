...
## 2026-06-09 - Add keyboard navigation to Toplevel dialogs
...
Explicitly call `.focus_set()` on primary entry fields when opening dialogs, and bind `<Return>`
events to primary actions, ensuring the action function signature includes an optional event
parameter (e.g., `def action(event=None):`).
