...
class PreviewAnchorMappingTests(unittest.TestCase):
    ...
    def test_render_paginated_html_document_renders_images_and_splits_pages(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            temp_dir = Path(td)
            img_path = temp_dir / 'test_extracted.png'
            # Write a dummy PNG header to avoid struct unpacking errors
            with open(img_path, 'wb') as f:
                f.write(b'\x89PNG\r\n\x1a\n' + b'\x00\x00\x00\x0dIHDR' + struct.pack('>IIBBBBB', 10, 10, 8, 2, 0, 0, 0))
            ...
            entries = [
                ChapterAssemblyEntry(
                    ...
                    path=workspace_dir / 'tests' / 'Ch01_Test.md',
                    content=(
                        '### Tieu de\n\n'
