...
def assemble_markdown(chapter_dir, output_path):
    ...
    for file_path in processed_files:
        ...
        # ⚡ Bolt: Fast line counting replacing splitlines
        lines_cnt = content.count('\n') + (1 if content and not content.endswith('\n') else 0)
        print(f'  [OK] {os.path.basename(file_path)}  ({lines_cnt} dong)')
...
def step_assemble():
    ...
    print(f'\n  => {MD_OUT}')
    # ⚡ Bolt: Fast line counting replacing splitlines
    final_lines_cnt = final.count('\n') + (1 if final and not final.endswith('\n') else 0)
    print(f'     {final_lines_cnt} dong | {kb} KB | {len(chapter_files)} chapters\n')
