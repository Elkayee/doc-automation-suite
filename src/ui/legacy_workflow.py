...
def assemble_markdown(chapter_dir, output_path):
    ...
    for file_path in processed_files:
        ...
        # ⚡ Bolt: Fast O(1) memory line counting replacing len(content.splitlines())
        lines = content.count('\n') + (1 if content and not content.endswith('\n') else 0)
        print(f'  [OK] {os.path.basename(file_path)}  ({lines} dong)')
...
def step_assemble():
    ...
    print(f'\n  => {MD_OUT}')
    # ⚡ Bolt: Fast O(1) memory line counting replacing len(final.splitlines())
    lines = final.count('\n') + (1 if final and not final.endswith('\n') else 0)
    print(f'     {lines} dong | {kb} KB | {len(chapter_files)} chapters\n')
