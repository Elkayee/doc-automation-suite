...
def assemble_markdown(chapter_dir, output_path):
    ...
    for file_path in processed_files:
        ...
        # ⚡ Bolt: Memory-efficient line counting replacing len(content.splitlines())
...
def step_assemble():
    ...
    print(f'\n  => {MD_OUT}')
    # ⚡ Bolt: Memory-efficient line counting replacing len(final.splitlines())
    final_lines = final.count('\n') + (1 if final and not final.endswith('\n') else 0)
    print(f'     {final_lines} dong | {kb} KB | {len(chapter_files)} chapters\n')
