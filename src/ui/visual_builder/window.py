...
class VisualBuilderWindow(tk.Toplevel):
    ...
    def _build_navigator(self):
        ...
        self.search_results_listbox = tk.Listbox(
            search_frame, font=('Consolas', 9), height=10, activestyle='none', cursor='hand2'
        )
    ...
    def _on_chapter_selected(self, _event=None):
        ...
        if index is not None:
            if self.chapter_listbox.itemcget(index, 'foreground') == 'gray':
                self.chapter_listbox.selection_clear(index)
                return
            if index < len(self.chapter_filenames):
