...
class VisualBuilderWindow(tk.Toplevel):
    ...
    def _build_navigator(self):
        ...
        self.chapter_content_frame = ttk.Frame(list_wrapper)
        self.chapter_content_frame.pack(fill='both', expand=True)
        self.chapter_listbox = tk.Listbox(
            self.chapter_content_frame, font=('Consolas', 10), activestyle='none', exportselection=False, cursor='hand2'
        )
        self.chapter_listbox.pack(side='left', fill='both', expand=True)
        scrollbar = ttk.Scrollbar(self.chapter_content_frame, orient='vertical', command=self.chapter_listbox.yview)
        ...
        self.chapter_empty_label = ttk.Label(
            list_wrapper, text='No chapters. Click "New Chapter" to start.', foreground='gray'
        )
        ...
        self.search_results_listbox = tk.Listbox(
            self.search_results_container, font=('Consolas', 9), height=10, activestyle='none', cursor='hand2'
        )
        self.search_results_listbox.bind('<Double-1>', self._open_selected_search_result)
        self.search_empty_label = ttk.Label(self.search_results_container, text='No matches found.', foreground='gray')
    ...
    def _load_chapter_list(self, select_filename: str | None = None):
        ...
        if not self.chapter_filenames:
            self.chapter_content_frame.pack_forget()
            self.chapter_empty_label.pack(expand=True)
