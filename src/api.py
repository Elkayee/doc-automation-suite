...
def compile_workspace(req: CompileRequest):
    ...
    # Security concern: User input paths can bypass relative path checks if they are absolute.
    # We use _secure_resolve to enforce that all generated files remain within the build_dir sandbox.
    try:
        final_md_out = _secure_resolve(build_dir, req.md_out) if req.md_out else build_dir / 'assembled.md'
        final_docx_out = (
            _secure_resolve(build_dir, req.docx_out) if req.docx_out else build_dir / f'{workspace_dir.name}.docx'
        )
